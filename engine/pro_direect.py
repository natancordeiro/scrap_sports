...
class ProDirect(Browser):
    ...
    def uk_to_br_shoe_sizes(self, marca=str, uk_sizes=list):
            ...
            if marca in uk_to_br_sizes:
                tabela = uk_to_br_sizes[marca]
                lista_br = []
                for uk in uk_sizes:
                    if uk in tabela:
                        lista_br.append(tabela[uk])
                    else:
                        lista_br.append(None)  # Caso o valor não seja encontrado na tabela de conversão, é adicionado None à lista
                return lista_br
            else:
                return None 
